...
def solve(state):
    ...
    s1 = s.replace('\'', '3')
    s2 = ''
    for it in range(0, len(s1)):
        s2 += s1[it]
        if (s1[it] != ' ' and s1[it] != '2' and s1[it] != '3') and (s1[it + 1] == ' '):
            s2 += '1'
    #print("answer:", s2)
    return Cube.solve(raw) ,s2
...
def color_detect(h, s, v):
    ...
    if (0 < h < 10 and s > 125):
        return 'red'
    #if (165 < h < 180 and v < 150):
     #   return 'red'
    elif (65 < h < 100 and v > 130 and s > 90):
        return 'green'
    elif (95 < h < 135 and s > 70):
        return 'blue'
    elif (15 < h < 80 and s < 160):
        return "yellow"
    elif (0 < h < 40 and v > 175):
        return 'orange'
    elif (s < 140):
        return 'white'
    return 'white'
...
def draw_stickers(frame, stickers, name):
    for x, y in stickers[name]:
        cv2.rectangle(frame, (x, y), (x, y), (0, 0, 255), 2)
        ...
        if k == 27:
            ...
            if len(set(check_state)) == 6:
                try:
                    ...
                    print(
                        "Error in side detection. Try again")
            else:
                print("Confirm if all faces are scanned and try again.")
                print("left to scan:", 6 - len(set(check_state)))
        cv2.imshow('preview', preview)
        cv2.imshow('frame', img[0:500, 0:500])

    cv2.destroyAllWindows()
    with serial.Serial('COM8', timeout=3) as uno:
        rndm = uno.readline()  # baþtaki boþ stringi alýr

        s2 = s2 + '\n'  # adds line after solution string

        uno.write(s2.encode())  # conveys string to Arduino
